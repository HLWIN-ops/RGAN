...
import torch
from misc.utils import mkdir, init_model, lab_conv, ROA
import numpy as np
import libmr
from models import *
...
def openmax(args, kdataloader_trn, kdataloader_tst, ukdataloader_tst, knownclass, Encoder, NorClsfier, testPy=False, test_black=False):
    Encoder.eval()
    NorClsfier.eval()
...
def precalc_weibull(args, dataloader_train, knownclass, Encoder, NorClsfier, device):
    ...
    activation_vectors = {}
    roa = ROA(Encoder, NorClsfier, args.imgsize)
    # if args.adv == 'PGD':
    ...
    # for _, (images, labels) in enumerate(dataloader):
        ...
        advimg = roa.exhaustive_search(images, labels, args.lr, 5, 5 , 5, 2, 2)
        with torch.no_grad():
