...
class Saver():
    def __init__(self, args, log_file):
        ...
        with open(self.log_name, "a") as log_file:
            now = time.strftime("%c")
            log_file.write('================ Training Loss (%s) seed(%d)================\n' % (now, args.manual_seed))
    ...
    def print_current_errors(self, epoch, i, errors, write=False):
    ...
    def save_best(self, close_best, close_best_epoch, open_best, open_best_epoch, seed):
        with open(self.log_name, "a") as log_file:
            now = time.strftime("%c")
            log_file.write('================ Training end time (%s) close_best(%.4f) epoch(%d) open_best(%.4f) epoch(%d) seed(%d)================\n\n\n' % (
            now, close_best, close_best_epoch, open_best, open_best_epoch, seed))
